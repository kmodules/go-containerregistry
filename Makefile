...
PRODUCT_NAME       := echo-image-digest
ENFORCE_LICENSE    ?=

GO_PKG   := kmodules.xyz
REPO     := $(notdir $(shell pwd))
BIN      := echo-image-digest
COMPRESS ?= no
...
SRC_PKGS := authn cmd name # directories which hold app source excluding tests (not vendored)
SRC_DIRS := $(SRC_PKGS) # directories which hold app source (not vendored)

DOCKER_PLATFORMS := linux/amd64 linux/arm64
BIN_PLATFORMS    := $(DOCKER_PLATFORMS) darwin/amd64 darwin/arm64 windows/amd64 windows/arm64
...
lint: $(BUILD_DIRS)
	...
	@docker run                                                 \
	    ...
	    $(BUILD_IMAGE)                                          \
	    golangci-lint run --enable $(ADDTL_LINTERS) --timeout=10m --exclude-files="generated.*\.go$\" --exclude-dirs-use-default --exclude-dirs=client,vendor,bin
